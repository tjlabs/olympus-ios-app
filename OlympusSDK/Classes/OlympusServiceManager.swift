...
//    var bleManager = OlympusBluetoothManager()
    ...
    var ambiguitySolvedIndex: Int = 0
    ...
    private actor TemporalResultActor {
        ...
        func startProcessing() -> Bool {
            guard !isProcessing && !mtrSkipFlag else {
                return false
            }
            isProcessing = true
            return true
        ...
        func canStartOnPointProcessing() -> Bool {
            return !isOnPointInProgress
        }
        
        func _startOnPointProcessing() -> Bool {
        ...
        func _endOnPointProcessing() {
            isOnPointInProgress = false
        }
        
        nonisolated func startOnPointProcessing() {
            Task { [weak self] in
                await self?._startOnPointProcessing()
            }
        }

        nonisolated func endOnPointProcessing() {
            Task { [weak self] in
                await self?._endOnPointProcessing()
            }
        }
        
        func _setMtrSkipFlag(_ flag: Bool) {
            mtrSkipFlag = flag
        }
        
        // 어디서든 동기로 호출 가능한 래퍼
        nonisolated func setMtrSkipFlagAsync(_ flag: Bool) {
            Task {
                await self._setMtrSkipFlag(flag)
            }
        }

        // 필요하면 읽기용 메서드는 async 그대로 두고
        func shouldSkipMtr() -> Bool {
            mtrSkipFlag
        }
        
        func getCurrentState() -> (matching: Bool, onPoint: Bool, processing: Bool, skipFlag: Bool) {
            return (isMatchingInProgress, isOnPointInProgress, isProcessing, mtrSkipFlag)
        }
    }
        
    
    private let temporalResultActor = TemporalResultActor()
    private var lastTrajMatchingIndex: Int = -1
    private var lastTrajMatchingFltResult: FineLocationTrackingFromServer = FineLocationTrackingFromServer()
    private var shouldRunTrajMatching: Bool = false
    ...
    private func handleRfd() {
        ...
        if (isSimulationMode) {
            ...
            if (bleLineCount < simulationBleData.count-1) {
                ...
                rssCompensator.refreshWardMaxRssiV2(bleTime: currentTime, bleData: self.bleAvg)
                let maxRssi = rssCompensator.getMaxRssiV2()
                let minRssi = rssCompensator.getMinRssiV2()
                ...
                rssCompensator.estimateNormalizationScaleV2(isGetFirstResponse: stateManager.isGetFirstResponse, isIndoor: stateManager.isIndoor, currentLevel: self.currentLevel, diffMinMaxRssi: diffMinMaxRssi, minRssi: minRssi)
...
//        print(getLocalTimeString() + " , (Olympus) checkBleEmptyState : isIndoor = \(stateManager.isIndoor) // checkBleEmptyStateThreshold = \(checkBleEmptyStateThreshold) // bleData = \(bleData)")
    ...
    func userVelocityTimerUpdate() {
        ...
        stateManager.checkEnterSleepMode(service: self.service, type: .UVD, isSleep: !unitDRInfo.isIndexChanged)
...
//                print(getLocalTimeString() + " , (Olympus) post : UVD \(unitDRInfoIndex)")
            ...
            if (KF.isRunning && KF.tuFlag && !self.isInRecoveryProcess) {
                ...
                let isNeedAnchorNodeUpdate = sectionController.extendedCheckIsNeedAnchorNodeUpdate(userVelocity: data, userHeading: self.temporalResult.absolute_heading, mode: self.runMode)
...
//                                    let inputPathType = runMode == OlympusConstants.MODE_PDR ? 0 : 1
//                                    let multipleNodeCandidates = OlympusPathMatchingCalculator.shared.getMultipleAnchorNodeCandidates(fltResult: tuResult, pathType: inputPathType, maskBuffer: self.userUniqueMaskBuffer)
...
//                print(getLocalTimeString() + " , (Olmypus) Request : isInRecoveryProcess")
    ...
    private func processPhase3ForAmbiguousTraj(currentTime: Int, mode: String, trajectoryInfo: [TrajectoryInfo], searchInfo: SearchInfo) {
        if (mode == OlympusConstants.MODE_PDR) {
            self.currentLevel = removeLevelDirectionString(levelName: self.currentLevel)
        }
        let levelArray = [self.currentLevel]
        
        displayOutput.phase = "3 // Traj"
...
//                    print(getLocalTimeString() + " , (Olympus) processPhase3ForAmbiguousTraj : ----------------------------------------------")
//                    print(getLocalTimeString() + " , (Olympus) processPhase3ForAmbiguousTraj : fltResult = \(fltResult.x),\(fltResult.y),\(fltResult.absolute_heading)")
                    // 임시
...
//                        print(getLocalTimeString() + " , (Olympus) phaseBreak : fltResult.scc < phaseBreakSCC = \(fltResult.scc)")
                        ...
                        var pmResult: FineLocationTrackingFromServer = fltResult
                        if (runMode == OlympusConstants.MODE_PDR) {
                            let pathMatchingResult = OlympusPathMatchingCalculator.shared.pathMatching(building: buildingName, level: levelName, x: fltResult.x, y: fltResult.y, heading: fltResult.absolute_heading, HEADING_RANGE: OlympusConstants.HEADING_RANGE, isUseHeading: false, pathType: 0, PADDING_VALUES: OlympusConstants.PADDING_VALUES)
                            pmResult.x = pathMatchingResult.xyhs[0]
                            pmResult.y = pathMatchingResult.xyhs[1]
                            pmResult.absolute_heading = pathMatchingResult.xyhs[2]
                        } else {
                            let pathMatchingResult = OlympusPathMatchingCalculator.shared.pathMatching(building: buildingName, level: levelName, x: fltResult.x, y: fltResult.y, heading: fltResult.absolute_heading, HEADING_RANGE: OlympusConstants.HEADING_RANGE, isUseHeading: true, pathType: 1, PADDING_VALUES: OlympusConstants.PADDING_VALUES)
                            pmResult.x = pathMatchingResult.xyhs[0]
                            pmResult.y = pathMatchingResult.xyhs[1]
                            pmResult.absolute_heading = pathMatchingResult.xyhs[2]
...
//                        print(getLocalTimeString() + " , (Olympus) processPhase3ForAmbiguousTraj : pmResult = \(pmResult.x),\(pmResult.y),\(pmResult.absolute_heading)")
...
//                        print(getLocalTimeString() + " , (Olympus) processPhase3ForAmbiguousTraj : dxdydh = \(propagationResult)")
//                        print(getLocalTimeString() + " , (Olympus) processPhase3ForAmbiguousTraj : propagatedResult = \(propagatedResult)")
                        ...
                        if (propagationResult.0) {
                            if (runMode == OlympusConstants.MODE_PDR) {
                                ...
                                let pathMatchingResult = OlympusPathMatchingCalculator.shared.pathMatching(building: buildingName, level: levelName, x: propagatedResult[0], y: propagatedResult[1], heading: propagatedResult[2], HEADING_RANGE: OlympusConstants.HEADING_RANGE, isUseHeading: true, pathType: 1, PADDING_VALUES: OlympusConstants.PADDING_VALUES)
                                propagatedResult = pathMatchingResult.xyhs
                            }
                        }
                        copiedResult.x = propagatedResult[0]
                        ...
                        displayOutput.serverResult[0] = propagatedResult[0]
...
//                        print(getLocalTimeString() + " , (Olympus) processPhase3ForAmbiguousTraj : do \(unitDRInfoIndex)")
                        ...
                        phaseController.phaseForceUpdate(phase: 6)
                    }
                ...
                let msg: String = getLocalTimeString() + " , (Olympus) Error : \(statusCode) Fail to request indoor position in Phase 3 For Ambiguous Traj"
                print(msg)
            }
            self.trajMisMatchOccured = false
        })
    ...
    func requestOlympusResult(trajectoryInfo: [TrajectoryInfo], trueHeading: Double, mode: String) {
        ...
        if (!stateManager.isBackground && isStartRouteTrack) {
            ...
            if isCorrelation.0 {
                ...
                print(getLocalTimeString() + " , (Olympus) Route Tracker : Stable Pos \(lastServerResult.x),\(lastServerResult.y),\(lastServerResult.absolute_heading)")
        ...
        if ((self.unitDRInfoIndex % RQ_IDX) == 0 && !stateManager.isBackground) {
            if (phaseController.PHASE == OlympusConstants.PHASE_1 || phaseController.PHASE == OlympusConstants.PHASE_3) {
                ...
                if (!isStartRouteTrack || isPhaseBreakInRouteTrack) {
                    if !trajMisMatchOccured { processPhase3(currentTime: currentTime, mode: mode, trajectoryInfo: phase3Trajectory, searchInfo: searchInfo) }
                }
            }
        }
    }
...
//        print(getLocalTimeString() + " , (Olympus) Request Phase 1 ~ 3 : \(input)")
        if (REGION_NAME != "Korea" && self.deviceModel == "iPhone SE (2nd generation)") { input.normalization_scale = 1.01 }
        OlympusNetworkManager.shared.postFLT(url: CALC_FLT_URL, input: input, userTraj: trajectoryInfo, searchInfo: searchInfo, completion: { [self] statusCode, returnedString, fltInput, inputTraj, inputSearchInfo in
//            print(getLocalTimeString() + " , (Olympus) Request Phase 1 ~ 3 : \(statusCode) , \(returnedString)")
            if (!returnedString.contains("timed out")) { stateManager.setNetworkCount(value: 0) }
            if (statusCode == 200) {
                let results = jsonToFineLocatoinTrackingResultFromServerList(jsonString: returnedString)
                let result = results.1.flt_outputs
...
//        print(getLocalTimeString() + " , (Olympus) Request Phase 5 : input = \(input)")
        ...
        OlympusNetworkManager.shared.postStableFLT(url: CALC_FLT_URL, input: input, userTraj: trajectoryInfo, nodeCandidateInfo: nodeCandidatesInfo, completion: { [self] statusCode, returnedString, fltInput, inputTraj, inputNodeCandidatesInfo in
            ...
            if (statusCode == 200) {
                let results = jsonToFineLocatoinTrackingResultFromServerList(jsonString: returnedString)
                let (useResult, fltResult) = ambiguitySolver.selectResult(results: results.1, nodeCandidatesInfo: inputNodeCandidatesInfo)
                print(getLocalTimeString() + " , (Olympus) Phase5 : useResult = \(useResult) // \(fltResult)")
                    ...
                    if (KF.isRunning && resultPhase.0 == OlympusConstants.PHASE_6) {
                        if (!(fltResult.x == 0 && fltResult.y == 0) && !buildingLevelChanger.isDetermineSpot && phaseController.PHASE != OlympusConstants.PHASE_2) {
                            ...
                            if (KF.muFlag) {
                                ...
                                // 위치 점프를 확인함
                                let intermediatePoints = checkResultJump(olympusResult: self.olympusResult, fltResult: muResult)
                                var jumpedNodes = [PassedNodeInfo]()
                                for point in intermediatePoints {
                                    let pathType = runMode == OlympusConstants.MODE_PDR ? 0 : 1
                                    let matchedNodeResult = OlympusPathMatchingCalculator.shared.getMatchedNodeWithCoord(fltResult: muResult, originCoord: point, coordToCheck: point, pathType: pathType, PADDING_VALUES: [1, 1, 1, 1])
                                    if matchedNodeResult.1 != -1 {
                                        let nodeInfo = PassedNodeInfo(nodeNumber: matchedNodeResult.1, nodeCoord: point, nodeHeadings: matchedNodeResult.2, matchedIndex: muResult.index, userHeading: olympusResult.absolute_heading)
...
//                            }
    ...
    private func checkResultJump(olympusResult: FineLocationTrackingResult, fltResult: FineLocationTrackingFromServer) -> [[Double]]  {
        var isJumped: Bool = false
        var intermediatePoints: [[Double]] = []
        
        let userX = olympusResult.x
        let userY = olympusResult.y
        let userDir = olympusResult.absolute_heading
        
        let resultX = fltResult.x
        let resultY = fltResult.y
        let resultDir = fltResult.absolute_heading
        ...
        if isJumped {
//            print(getLocalTimeString() + " , (Olympus) checkResultJump : \(unitDRInfoIndex) , \(userDir) degree , [\(userX),\(userY)] -> [\(resultX),\(resultY)] with dist \(distance) // isJumped \(isJumped)")
...
//            print(getLocalTimeString() + " , (Olympus) checkResultJump : intermediatePoints = \(intermediatePoints)")
        }
    ...
    func safeMakeTemporalResultWrapper(input: FineLocationTrackingFromServer,
        ...
        // Actor를 통한 thread-safe 플래그 체크 및 설정
        Task { [weak self] in
            guard let self = self else { return }
            
            // Actor를 통한 안전한 플래그 체크
            guard await self.temporalResultActor.startProcessing() else {
                print(getLocalTimeString() + " , (OlympusServiceManager) safeMakeTemporalResultWrapper : processing already in progress")
                return
            }
            
            defer {
                Task {
                    await self.temporalResultActor.endProcessing()
                }
            }
            
            await MainActor.run {
                self.makeTemporalResult(
                    input: input,
                    isStableMode: isStableMode,
                    mustInSameLink: mustInSameLink,
    ...
    func makeTemporalResult(input: FineLocationTrackingFromServer, isStableMode: Bool, mustInSameLink: Bool, updateType: UpdateNodeLinkType, pathMatchingType: PathMatchingType, jumpedNodes: [PassedNodeInfo] = [], runInUvdTimer: Bool = false) {
        var result = input
        ...
        if ((result.x != 0 || result.y != 0) && result.building_name != "" && result.level_name != "") {
            ...
            let condition = pathMatchingType == .NARROW && runMode == OlympusConstants.MODE_PDR ? true : isUseHeading
            if (condition && isStableMode && !self.isPhaseBreak) {
            ...
            if (KF.isRunning) {
                OlympusPathMatchingCalculator.shared.updateNodeAndLinkInfo(uvdIndex: resultIndex, currentResult: result, currentResultHeading: self.curTemporalResultHeading, pastResult: preTemporalResult, pastResultHeading: preTemporalResultHeading, pathType: pathTypeForNodeAndLink, updateType: updateType, jumpedNodes: jumpedNodes)
            ...
            if (isStableMode) {
                ...
                if !trajMisMatchOccured && resultIndex - self.trajMisMatchIndex > 30 && runInUvdTimer {
                    // Actor를 통한 thread-safe 플래그 체크
                    Task { [weak self] in
                        guard let self = self else { return }
                        
                        // Actor를 통해 매칭 시작 가능 여부 확인
                        guard await self.temporalResultActor.startMatching() else {
                            print(getLocalTimeString() + " , (OlympusServiceManager) checkForTrajMatching : matching already in progress")
                            return
                        }
                        
                        // 배열들의 안전한 복사본 생성
                        await MainActor.run {
                            let userMaskBufferCopy = Array(self.userMaskBuffer)
                            let unitDRInfoBufferCopy = Array(self.unitDRInfoBuffer)
                            let linkCoordCopy = Array(self.KF.linkCoord)
                            let linkDirectionsCopy = Array(self.KF.linkDirections)
                            let currentAmbiguitySolvedIndex = self.ambiguitySolvedIndex
                            let currentRunMode = self.runMode
                            
                            Task.detached { [weak self] in
                                guard let self = self else {
                                    Task {
                                        await self?.temporalResultActor.endMatching()
                                    }
                                    return
                                }
                                ...
                                guard let comparingResult = checkForTrajMatching(
                                    index: resultIndex,
                                    ambiguitySolvedIndex: currentAmbiguitySolvedIndex,
                                    fltResult: result,
                                    userMaskBuffer: userMaskBufferCopy,
                                    unitDRInfoBuffer: unitDRInfoBufferCopy,
                                    linkCoord: linkCoordCopy,
                                    linkDirections: linkDirectionsCopy,
                                    mode: currentRunMode
                                ) else {
                                    return
                                }
                                    ...
                                    if !isInLevelChangeArea && dTime > 20*1000 && self.stateManager.isIndoor && self.phaseController.PHASE > 4 {
                                        ...
                                        if comparingResult.0 == .STRAIGHT {
                                            ...
                                            if inputPoints.count > 1 {
                                                self.temporalResultActor.startOnPointProcessing()
                                                ...
                                                OlympusNetworkManager.shared.postOPE(url: CALC_OPE_URL, input: opeInput, completion: { statusCode, returnedString in
                                                    ...
                                                    if statusCode == 200 {
                                                        ...
                                                        if decoded.0 {
                                                            ...
                                                            if let bestPoint = pointResults.max(by: { $0.ccs < $1.ccs }) {
                                                                ...
                                                                result.x = xyh[0]
                                                                result.y = xyh[1]
                                                                self.temporalResult.x = xyh[0]
                                                                self.temporalResult.y = xyh[1]
                                                                ...
                                                                print(getLocalTimeString() + " , (OlympusServiceManager) checkForTrajMatching : STRAIGHT newCoord = \(newCoord)")
                                                    ...
                                                    self.temporalResultActor.endOnPointProcessing()
                                                })
                                            }
                                        } else {
                                            ...
                                            if !xyh.isEmpty {
                                                ...
                                                self.temporalResult.x = xyh[0]
                                                self.temporalResult.y = xyh[1]
                                                self.temporalResult.absolute_heading = xyh[2]
                                                let newCoord = [xyh[0], xyh[1]]
                                                print(getLocalTimeString() + " , (OlympusServiceManager) checkForTrajMatching : TURN newCoord = \(newCoord)")
                                                self.KF.updateTuResult(x: newCoord[0], y: newCoord[1])
                                                self.KF.setLinkInfo(coord: newCoord, directions: OlympusPathMatchingCalculator.shared.getPathMatchingHeadings(building: result.building_name, level: result.level_name, x: newCoord[0], y: newCoord[1], PADDING_VALUE: 0.0, mode: self.runMode))
                    ...
                    if self.isDRMode {
                        if(checkIsBadCase(userMaskBuffer: self.userMaskBuffer) && !self.isPhaseBreak) {
                            if buildingLevelChanger.checkCoordInSectorDRModeArea(fltResult: result) {
                                print(getLocalTimeString() + " , (Olympus) Info : phase break in bad case")
                ...
                if diffXY >= 12 && pathMatchingType == .NARROW {
                    ...
                    if !onPoints.isEmpty {
                        ...
                        self.temporalResultActor.setMtrSkipFlagAsync(true)
                        let opeInput = OnPointEstimation(user_id: self.user_id, mobile_time: getCurrentTimeInMilliseconds(), sector_id: self.sector_id, building_name: result.building_name, level_name: result.level_name, operating_system: OlympusConstants.OPERATING_SYSTEM, normalization_scale: OlympusConstants.NORMALIZATION_SCALE, device_min_rssi: Int(OlympusConstants.DEVICE_MIN_RSSI), standard_min_rssi: Int(OlympusConstants.STANDARD_MIN_RSS), points: onPoints)
                        OlympusNetworkManager.shared.postOPE(url: CALC_OPE_URL, input: opeInput, completion: { statusCode, returnedString in
                            if statusCode == 200 {
                                let decoded = jsonToOnPointEstimationResult(jsonString: returnedString)
                                if decoded.0 {
                                    ...
                                    if let bestPoint = pointResults.max(by: { $0.ccs < $1.ccs }) {
                                        ...
                                        if !OlympusConstants.DEFAULT_HEADINGS.contains(result.absolute_heading) {
                                            ...
                                            let diffNorm = sqrt(diffX*diffX + diffY*diffY)
                                            if diffNorm >= 2 {
                                                self.KF.updateTuResult(x: result.x, y: result.y)
                            ...
                            self.temporalResultActor.setMtrSkipFlagAsync(false)
                        })
                    }
                }
    ...
    private func checkInPhase3ForAmbiguousTraj(result: FineLocationTrackingResult, mode: String) -> Bool {
        if (mode == OlympusConstants.MODE_PDR) {
            return false
        }
        ...
        if !key.contains("Convensia_B1") {
            return false
        }
        
        let phase3ForAmbiguousTrajArea: [Double] = [294, 167, 331, 169]
        let xMin = phase3ForAmbiguousTrajArea[0]
        let yMin = phase3ForAmbiguousTrajArea[1]
        let xMax = phase3ForAmbiguousTrajArea[2]
        let yMax = phase3ForAmbiguousTrajArea[3]
