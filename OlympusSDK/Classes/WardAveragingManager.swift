...
final class WardAveragingManager {
    ...
    private let missingRssi: Double
    ...
    init(bufferSize: Int, missingRssi: Double = -100.0) {
    ...
    func updateEpoch(bleData: [String: Double]) -> [String: Double] {
        ...
        for id in knownBeaconIds {
            if let arr = buffers[id], !arr.isEmpty, bleData[id] != nil {
                ...
                let sum = arr.reduce(Double(0), +)
                result[id] = sum / Double(arr.count)
            } else {
