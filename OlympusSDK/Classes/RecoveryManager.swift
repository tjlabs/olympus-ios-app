            ...
            for recoveryTraj in recoveryTrajList {
                group.addTask { [self, sectorId = self.sectorId] in
                    ...
                    for cand in recentLandmarkCands {
                        ...
                        JupiterLogger.i(tag: "RecoveryManager", message: "(trackWith2PeaksAsync) shiftedTraj.count: \(shiftedTraj.count) -> residualIndices \(residualIndices)")
...
//                            let loss = dist1 + dist2v + dist3
    ...
    private func trackWith2PeaksAsync(recoveryTrajList: [[RecoveryTrajectory]],
        ...
        JupiterLogger.i(tag: "RecoveryManager", message: "(trackWith2PeaksAsync) curPmResult [\(curPmResult.x),\(curPmResult.y)] is in \(curLinkForConnectionCheck?.id) , isInNode = \(isInNode)")
                ...
                group.addTask { [self,
                    ...
                    for candTuple in selectedCands {
                        ...
                        if !distBestOnly {
                            ...
                            if headLink.group_id == curLink.group_id && !isHeadInNode {
                                JupiterLogger.i(tag: "RecoveryManager", message: "(trackWith2PeaksAsync) headPos = [\(head.x),\(head.y)]")
                            ...
                            let reachable = self.isLinkReachableWithGroupSwitchLimit(nodeData: nodeData,
                                                                                    ...
                                                                                    to: headLink,
                                                                                    maxGroupSwitches: 1)
                            JupiterLogger.i(tag: "RecoveryManager", message: "(trackWith2PeaksAsync) reachable fail // curLink:\(curLink.id) -> headLink:\(headLink.id)")
                            if !reachable {
                                continue
                            }
                        }
...
//                            let loss = (dist1 + dist2v + dist3) * penalty
                        ...
                        JupiterLogger.i(tag: "RecoveryManager", message: "(trackWith2PeaksAsync) cand:\(cand.x),\(cand.y), loss_g_d:\(loss_g_d), loss_g_h:\(loss_g_h), loss_l:\(loss_l), loss:\(loss), penalty:\(penalty)")
