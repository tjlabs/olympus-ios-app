...
class NaviViewController: UIViewController, Observer {
    
    func update(result: OlympusSDK.FineLocationTrackingResult) {
        let user = TJLabsUserCoordinate(building: result.building_name, level: result.level_name, x: result.x, y: result.y, heading: result.absolute_heading, velocity: result.velocity)
        self.mapView.updateResultInMap(result: user)
    }
...
//    var mode: String = "pdr"
    ...
    func setupNaviView() {
        ...
        mapView.initialze(region: .KOREA, sectorId: self.sector_id)
    }
